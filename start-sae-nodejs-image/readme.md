...
# 使用SAE组件快速部署 nodejs 镜像
## 本地快速体验
通过该应用，您可以简单快速的使用SEA组件部署 nodejs 镜像。

- 下载命令行工具：`npm install -g @serverless-devs/s`
- 初始化一个模版项目：`s init start-sae-nodejs-image`
- 执行`s deploy`命令，自动将镜像部署到Serverless应用引擎SAE，并绑定公网SLB，让您的应用可以被公网访问。

## Node.js 镜像说明
code文件夹下是制作镜像用到的文件，镜像地址为：registry.cn-hangzhou.aliyuncs.com/namespace4sae/node-demo:v1。您也可以按照[以下步骤](https://help.aliyun.com/document_detail/432780.html)制作自己的镜像。
...
```yaml
code:
  image: registry.cn-hangzhou.aliyuncs.com/namespace4sae/node-demo:v1
port: 8080
```
将s.yaml文件中的code.image的地址替换为您自己的镜像地址，并设置容器端口port，执行`s deploy`命令自动将镜像部署到SAE，执行结果示例如下：
```
sae-test: 
  namespace: 
    id:          cn-hangzhou:test
